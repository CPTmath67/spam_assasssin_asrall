...
score pour définir une marge de définitions des SPAMs(valeurs ''require``). Une fois ce score ateint le mail est considéré comme SPAM.
...
\begin{figure}[b]
 ...
 \label{fig:ex_score}
 \caption{Exemple d'entête réécrit par SpamAssassin}
\end{figure}
...
\begin{figure}
 ...
 \caption{Exemle de configuration basique de SpamAssassin}
...
avec SpamAssassin juste après leurs reception par le demon SMTP. Mais la façon la plus répandue est d'utiliser procmail qui se charge
de filtrer satiquement les mails et de la déposer dans leurs boites respectives. 

\subsubsection{procmail}
